...
def create_post_embedding(post_data, image_path):
    """Create embedding directly in vector DB (no post creation)"""
    try:
        with open(image_path, 'rb') as f:
            ...
            # Use the endpoint to generate embedding and store in vector DB
            response = requests.post(
                f'{BASE_URL}/api/embeddings/create',
                files=files,
                data=data,
                timeout=60
            )
            ...
            if response.status_code == 201:
                ...
                # Fallback: use the matching endpoint to create embedding
                response = requests.post(
...
def seed_database():
    ...
    for i, post in enumerate(TEST_POSTS, 1):
        ...
        print(f"   🚀 Creating embedding in vector DB...")
        print(f"   🤖 AI processing (may take 2-5 seconds)...")
        
        success = create_post_embedding(post, filename)
    ...
    print("=" * 60)
    print(f"\n✅ Created {len(created_posts)} static posts:\n")
    
    for p in created_posts:
        type_emoji = "🔴" if p['type'] == 'lost' else "🟢"
        print(f"   {type_emoji} {p['title']} ({p['type'].upper()}) - ID: {p['id']}")
